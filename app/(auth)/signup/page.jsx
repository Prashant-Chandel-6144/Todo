...
function PasswordField({ label, placeholder, value, onChange, error, strength }) {
  const [focused, setFocused] = useState(false);
  const [visible, setVisible] = useState(false);
  return (
    <div>
      <label style={{ fontSize:12, color:C.muted, display:"block", marginBottom:7, fontWeight:600, letterSpacing:"0.05em", textTransform:"uppercase" }}>{label}</label>
      <div style={{ position:"relative" }}>
        <input
          ...
          style={{
            ...
            borderRadius:9, padding:"11px 44px 11px 14px", fontSize:14,
        ...
        <button type="button" className="eye-btn" onClick={() => setVisible(v => !v)}
          ...
          style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", color:C.muted, padding:2, display:"flex", alignItems:"center", justifyContent:"center" }}
        >
      ...
      {strength !== undefined && value.length > 0 && (
        <div style={{ marginTop:8 }}>
          ...
          <p style={{ fontSize:10, color: strength <= 1 ? C.danger : strength <= 2 ? C.accent : C.success }}>
        ...
        <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:"48px 24px" }}>
          <div style={{ width:"100%", maxWidth:480, animation:"fadeUp 0.5s ease" }}>
            ...
            <div style={{ textAlign:"center", marginBottom:36 }}>
              <div style={{ width:52, height:52, borderRadius:15, margin:"0 auto 20px", background:`linear-gradient(135deg, ${C.accent}, #ff7c3a)`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:24, boxShadow:`0 8px 24px ${C.accent}44` }}>T</div>
              <h1 style={{ fontSize:32, fontWeight:900, letterSpacing:"-0.04em", marginBottom:10, color:C.text }}>Create account</h1>
              <p style={{ fontSize:14, color:C.muted, lineHeight:1.6 }}>Start organizing your tasks for free</p>
            </div>
            ...
            <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:18, padding:"32px 32px 28px", boxShadow:"0 24px 60px rgba(0,0,0,0.5)" }}>
              ...
              <div style={{ display:"flex", gap:10, marginBottom:24 }}>
                {[{ icon:"G", label:"Google" }, { icon:"⌥", label:"GitHub" }].map(({ icon, label }) => (
                  <button key={label} type="button" className="oauth-btn"
                    onClick={() => alert(`${label} OAuth coming soon!`)}
                    style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", gap:8, padding:"11px", borderRadius:9, cursor:"pointer", background:C.bg, border:`1px solid ${C.border}`, color:C.text, fontSize:13, fontWeight:600, fontFamily:"inherit" }}
              ...
              {serverError && (
                <div style={{ background:"#e0525212", border:`1px solid ${C.danger}40`, borderRadius:9, padding:"12px 14px", marginBottom:20, fontSize:13, color:C.danger, display:"flex", alignItems:"center", gap:8 }}>
              ...
              <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:16 }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                  <TextField label="First Name" placeholder="Alex" value={form.fname} onChange={update("fname")} error={errors.fname} />
                  <TextField label="Last Name"  placeholder="Smith" value={form.lname} onChange={update("lname")} error={errors.lname} />
                </div>
                <TextField label="Email" type="email" placeholder="alex@example.com" value={form.email} onChange={update("email")} error={errors.email} />
                <PasswordField label="Password" placeholder="Create a password" value={form.password} onChange={update("password")} error={errors.password} strength={strength} />
                <PasswordField label="Confirm Password" placeholder="Repeat your password" value={form.confirm} onChange={update("confirm")} error={errors.confirm} />
